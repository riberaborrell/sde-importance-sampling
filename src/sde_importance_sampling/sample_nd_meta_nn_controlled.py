...
def get_parser():
    ...
    parser.add_argument(
        ...
        choices=['cum', 'ind'],
        default='cum',
        help='Type of metadynamics algorithm. Default: meta',
    )
    return parser
...
def main():
    ...
    # get meta sampling
    meta = sample.get_metadynamics_nn_sampling(args.dt_meta, args.sigma_i_meta,
                                            args.meta_type, args.k_meta, args.N_meta)
    ...
    if not args.load:
        theta = meta.thetas[-1]
        func.model.load_parameters(theta)
