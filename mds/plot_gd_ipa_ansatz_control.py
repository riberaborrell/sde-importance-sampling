...
def get_parser():
    ...
    parser.add_argument(
        '--ansatz',
        dest='ansatz',
        choices=['control', 'value_f'],
        default='control',
        help='Choose what to parametrize',
    )
    parser.add_argument(
        '--sigma',
        dest='sigma',
...
def main():
    ...
    # set gd path
    gd_stamp = 'gd-ipa-ansatz-{}-{}'.format(args.ansatz, args.theta_init)
    ...
    # get gd
    gd = np.load(os.path.join(gd_path, 'gd-ipa.npz'))
    omega_h = gd['omega_h']
    ...
    # plot all epochs
    plot_gd_tilted_potentials(gd_path, omega_h, potential, F_opt, F)
    plot_gd_losses(gd_path, value_f, loss)
    plot_gd_losses_bar(gd_path, value_f, loss)
